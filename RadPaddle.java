...
public class RadPaddle extends JFrame implements ActionListener, ChangeListener {
	...
	public RadPaddle() {
		...
		b = new Body(ballP, ballV, new Vec2D(0, -3), ballV, 0, mass);
		...
		gbc = new GridBagConstraints(col, row, numCol, numRow, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(100, 100, 100, 200), 0, 0);
	...
	class GameUpdater implements ActionListener {
		...
		Body b, paddle;
		public double timeIncrement = 0.06;
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (b.getVelocity().getX() > 0.0 && b.getPosition().getX() + b.radius >= width) {
			...
			if (b.getVelocity().getY() > 0.0 && b.getPosition().getY() + b.radius >= height) {
				b.getVelocity().setY(-(b.getVelocity().getY()));
			}

			// Determine if ball collides with the bottom wall.
			// If it does, change the z-velocity of the ball.
			if (b.getVelocity().getY() < 0.0 && b.getPosition().getY() + b.radius <= 0.0) {
				b.getVelocity().setY(-(b.getVelocity().getY()));
			}
			 ...
			 * b.getPosition().setX(b.getPosition().getX() +
			 * timeIncrement*b.getVelocity().getX());
			 * b.getPosition().setY(b.getPosition().getY() +
			 * timeIncrement*b.getVelocity().getY());
			 */
	...
	public static void main(String args[]) {
		...
		while(!false){
			System.out.println(gui.b.getPosition());
		}
	}
