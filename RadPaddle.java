...
public class RadPaddle extends JFrame implements ActionListener, ChangeListener {
	...
	double mass = 10, coefficient=.90;//.94 coefficient of a steel wall

	public RadPaddle() {

		Vec2D ballV = new Vec2D(100, -71), ballP = new Vec2D(100, 100), paddleP = new Vec2D(0, 100);
		b = new Body(ballP, ballV, /*new Vec2D(0, -9.81)*/new Vec2D(0,0), new Vec2D(0,0), 0, mass);
		b2 = new Body(new Vec2D(200,100), new Vec2D(0,0), /*new Vec2D(0, -9.81)*/new Vec2D(0,0), new Vec2D(0,0), 0, mass);
		paddle = new Body(paddleP, null, null, null, 0, 0);
		b.coefficientOfRestitution=.7;
		b.radius = 10.0;
		b2.radius = 10.0;
		paddle.height = 50;
	...
	private void updateDisplay() {
		...
		// Update the position of the second ball on the screen. NUEVO!!!!!!!!
				g.setColor(Color.BLUE);
				int xPos = (int) (b2.getPosition().getX() - b2.radius);
				int zPos = (int) (height - b2.radius - b2.getPosition().getY());
				g.fillOval(xPos, zPos, 2 * (int) (b2.radius), 2 * (int) (b2.radius));
...
/*CÓDIGO PARA LA SEGUNDA PELOTA*/			
			
			// Determine if ball collides with the top wall.
						// If it does, change the Y-velocity of the ball.
						if (Contact.testBoundaryOverlap(b2, limit, Contact.top)) {
							Contact.elasticCollisionHandler(b2,coefficient, Contact.Y);
//							b.getVelocity().setY(-(b.getVelocity().getY()));
						}

						// Determine if ball collides with right wall.
						// If it does, change the x-velocity of the ball.
						if (Contact.testBoundaryOverlap(b2, limit, Contact.right)) {
							Contact.elasticCollisionHandler(b2,coefficient, Contact.X);
//							b.getVelocity().setX(-(b.getVelocity().getX()));
						}

						// Determine if ball collides with the bottom wall.
						// If it does, change the Y-velocity of the ball.
						if (Contact.testBoundaryOverlap(b2, limit, Contact.bottom)) {
							Contact.elasticCollisionHandler(b2,coefficient, Contact.Y);
//							b.getVelocity().setY(-(b.getVelocity().getY()));
						}
						
						if (Contact.testBoundaryOverlap(b2, limit, Contact.left)) {
							Contact.elasticCollisionHandler(b2,coefficient, Contact.X);
						}
						
			if(Contact.testBodiesOverlap(b, b2)) {
				Contact.elasticCollisionHandler(b, b2);
			}

			// Compute the new location of the ball.

			b.updateConstAcc(timeIncrement);
			b2.updateConstAcc(timeIncrement);
